 ...
 *
 * This file contains the Box.net API, as retrieved from http://bit.ly/BoxGithub
 * 
 * The Box.net API is distributed under a MIT license and was modified by
 * AkeebaBackup.com as per the terms of the license. The rest of the header is
...
class AEUtilBox extends AEAbstractObject
{
	...
	public static function &getInstance($api_key = null)
	{
		...
		{
			$instance = new AEUtilBox($api_key);
		}
		return $instance;
	}
	...
	public function __construct($api_key = '') {
		if(empty($this->api_key) && empty($api_key)) {
			$this->setError('Invalid API Key. Please provide an API Key when creating an instance of the class, or by setting Box_Rest_Client->api_key');
		}
		else {
	...
	public function upload(Box_Client_File &$file, array $params = array(), $upload_then_delete = false) {
		...
		$skipRename = array_key_exists('skiprename', $params) ? $params['skiprename'] : 0;
		if(($file->attr('localpath') != $new_localpath) && !$skipRename) {
